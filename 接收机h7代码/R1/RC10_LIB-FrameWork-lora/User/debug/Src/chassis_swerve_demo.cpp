...
void Chassis_Swerve::init_swerve()
{
    ...
    static const struct WheelInit {
        ...
        void *gpio_port; uint16_t gpio_pin;
    ...
    for (uint8_t i = 0; i < 4; ++i) 
    {
        ...
        if (w.theta_rad < 0.0f) w.theta_rad += 2.0f * PI;
...
void Chassis_Swerve::kinematic_calc()
{
    ...
    if (mag > max_linear_vel_) v_body = v_body * (max_linear_vel_ / mag);
    if (fabsf(omega_z) > max_angular_vel_) omega_z = copysignf(max_angular_vel_, omega_z);
    ...
    if (is_zero_speed) 
    {
        for (uint8_t i = 0; i < 4; ++i) drive_speed[i] = 0.0f;
        return;
    }
...
void Chassis_Swerve::repostioning()
{
    for (uint8_t i = 0; i < 4; ++i) {
        ...
        if (!w.homing_sensor_active_high) sensor_raw = !sensor_raw;

        float motor_total_deg = 0.0f;
        if (steer_motors[i] != nullptr) motor_total_deg = steer_motors[i]->getTotalAngle();
...
void Chassis_Swerve::update()
{
    ...
    for (uint8_t i = 0; i < 4; ++i) 
    {
        ...
        if (w.homing_state == HomingState::kSearch) 
        {
            ...
            if (drive_motors[i]) drive_motors[i]->setTargetRPM(0.0f);
            continue;
        }
