...
uint16_t rx_len = 0;          // �ĳ�16λ����ȫ
uint8_t rx_flag = 0;
uint32_t time_use=0;
uint32_t time_start = 0;      
uint8_t tx_buf[25] ={0x01,0x02,0x03,0x04,0x05};
uint16_t tx_cnt=0;
...
//  HAL_UART_Transmit(&huart1, tx_buf, sizeof(tx_buf), 100);
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
		if(rx_flag==1)
		{
//			time_use=HAL_GetTick()-time_start;
...
void HAL_UARTEx_RxEventCallback(UART_HandleTypeDef *huart_, uint16_t size_)
{
    rx_len = size_;
    rx_flag = 1;
		if(rx_buffer[0]==0x05&&rx_buffer[1]==0x04&&rx_buffer[2]==0x03&&rx_buffer[3]==0x02&&rx_buffer[4]==0x01)
		{
			tx_cnt=1000;
		}   
}
...
{
    if (htim->Instance == TIM1)  // 注意是TIM1，不是TIM2
    {   
			if(tx_cnt>0)
			{
