...
// 在外部或者文件上方定义全局变量以便用调试器查看
volatile uint32_t rf_tx_cnt = 0;
volatile uint32_t rf_last_tx_cyccnt = 0;
volatile uint32_t rf_tx_interval_us = 0;

void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin)
{
    if(GPIO_Pin == TXSX1281_AUX_Pin)
    {
        ...
        rf_tx_interval_us = (current_cyccnt - rf_last_tx_cyccnt) / 168; 
        
        // 更新上一拍
        rf_last_tx_cyccnt = current_cyccnt;
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim)
{
  ...
  {
    timer_tick_count++;
    if (timer_tick_count >= 100) { // TIM1以1ms为基准累计100次，每100ms发送一帧
        timer_tick_count = 0;
